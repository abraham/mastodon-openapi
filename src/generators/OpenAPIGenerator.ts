...
class OpenAPIGenerator {
  ...
  private resolveEnumNameClashes(spec: OpenAPISpec): void {
    ...
    for (const [componentName, schema] of Object.entries(
      spec.components.schemas
    )) {
      ...
      if (enumSchema.type === 'string' && Array.isArray(enumSchema.enum)) {
        ...
        const propertyName =
          this.extractPropertyNameFromComponentName(componentName);
        const entityName =
          this.extractEntityNameFromComponentName(componentName);
      ...
      if (distinctEnumSignatures.size > 1) {
        ...
        for (const comp of components) {
          if (comp.entityName && comp.propertyName) {
            const sanitizedEntity = comp.entityName.replace(
              /[^a-zA-Z0-9_]/g,
              '_'
            );
   ...
   */
  private extractEntityNameFromComponentName(
    componentName: string
  ): string | undefined {
  ...
  private consolidateIdenticalEnumValues(
    ...
    for (const [enumSignature, group] of enumValueGroups) {
      ...
      const optimalName = this.generateOptimalEnumName(
  ...
  private extractPropertyNameFromComponentName(componentName: string): string {
    ...
    const entityAttrMatch = componentName.match(
      /^[A-Z][a-zA-Z0-9]*([A-Z][a-zA-Z0-9]*)Enum$/
    );
    if (entityAttrMatch) {
   ...
   */
  private generateOptimalEnumName(
    componentNames: string[],
    propertyName?: string
  ): string {
    if (!propertyName) {
      // Extract from the first component name
      propertyName = this.extractPropertyNameFromComponentName(
        componentNames[0]
      );
    }
  ...
  private extractEntityEnumsToComponents(
    ...
    const propertyEnumInfo = new Map<
      string,
      {
  ...
  private collectEnumInfo(
    ...
    const existing = propertyEnumInfo
      .get(propName)!
      .find((info) => info.enumSignature === enumSignature);
    if (existing) {
