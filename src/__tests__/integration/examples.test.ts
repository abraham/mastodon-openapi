...
describe('Example Generation Integration', () => {
          ...
          if (content && typeof content === 'object') {
            for (const [, mediaContent] of Object.entries(content)) {
              if (
                ...
                'example' in mediaContent
              ) {
                foundExample = true;
                expect(mediaContent.example).toBeDefined();
                console.log(
                  `Found example for ${statusCode} response in account registration:`,
  ...
  test('should gracefully handle entities without examples', () => {
    ...
    if (schema.components?.schemas) {
      for (const [, entitySchema] of Object.entries(
        schema.components.schemas
      )) {
        if (
          entitySchema &&
          typeof entitySchema === 'object' &&
          'example' in entitySchema
        ) {
