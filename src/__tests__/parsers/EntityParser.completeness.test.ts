...
describe('EntityParser - Deep Attribute Analysis', () => {
  ...
  test('should analyze Account entity for any missing attributes', () => {
    ...
    if (accountEntity) {
      console.log(`Account entity has ${accountEntity.attributes.length} attributes`);
      ...
      console.log(`Manual count found ${manualAttributeNames.length} attributes`);
      console.log('Manual attributes:', manualAttributeNames.slice(0, 10).join(', ') + '...');

      const parsedAttributeNames = accountEntity.attributes.map(a => a.name);
      console.log('Parsed attributes:', parsedAttributeNames.slice(0, 10).join(', ') + '...');

      // Find any manual attributes not in parsed list
      const missing = manualAttributeNames.filter(name => !parsedAttributeNames.includes(name));
      if (missing.length > 0) {
      ...
      const extra = parsedAttributeNames.filter(name => !manualAttributeNames.includes(name));
      if (extra.length > 0) {
  ...
  test('should check CohortData entity from Admin_Cohort.md', () => {
    ...
    if (cohortEntity) {
      console.log(`Admin::Cohort entity has ${cohortEntity.attributes.length} attributes`);
      console.log('Admin::Cohort attributes:', cohortEntity.attributes.map(a => a.name).join(', '));
    }

    if (cohortDataEntity) {
      console.log(`CohortData entity has ${cohortDataEntity.attributes.length} attributes`);
      console.log('CohortData attributes:', cohortDataEntity.attributes.map(a => a.name).join(', '));
    }
  });
});