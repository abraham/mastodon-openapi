...
describe('EntityParser - Draft File Handling', () => {
  ...
  test('should only skip files with draft: true (not other truthy values)', () => {
    ...
    const entities = parser.parseAllEntities();
    expect(entities.length).toBe(90); // Exact count after removing EncryptedMessage (increased due to extracted nested entities + Admin::DimensionData + OAuthServerConfiguration)
  });
});
