...
describe('EntityParser - Draft File Handling', () => {
  ...
  test('should only skip files with draft: true (not other truthy values)', () => {
    ...
    const entities = parser.parseAllEntities();
    expect(entities.length).toBe(99); // Exact count after removing EncryptedMessage and entities from blocked files (increased due to extracted nested entities + Admin::DimensionData + DiscoverOauthServerConfigurationResponse + OEmbedResponse + PartialAccountWithAvatar + new doc entities)
  });
});
