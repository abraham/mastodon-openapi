...
describe('JsonExampleAnalyzer', () => {
  ...
  test('should analyze simple JSON object', () => {
    ...
    expect(attributes.find(a => a.name === 'followers_count')?.type).toBe('Integer');
    expect(attributes.find(a => a.name === 'created_at')?.type).toBe('String (Date)');
  ...
  test('should handle arrays', () => {
    ...
    expect(emojisAttr?.nestedObject?.find(n => n.name === 'shortcode')?.type).toBe('String');
    expect(emojisAttr?.nestedObject?.find(n => n.name === 'url')?.type).toBe('String (URL)');
  ...
  test('should handle nested objects', () => {
    ...
    expect(sourceAttr?.nestedObject?.find(n => n.name === 'privacy')?.type).toBe('String');
    expect(sourceAttr?.nestedObject?.find(n => n.name === 'sensitive')?.type).toBe('Boolean');
    
    const fieldsNested = sourceAttr?.nestedObject?.find(n => n.name === 'fields');
  ...
  test('should convert to EntityAttribute format', () => {
    ...
    expect(entityAttributes.find(a => a.name === 'username')?.type).toBe('String');
    expect(entityAttributes.find(a => a.name === 'locked')?.type).toBe('Boolean');
    
    for (const attr of entityAttributes) {
      expect(attr.description).toContain('Attribute discovered from JSON example');
    }
  });
  ...
  test('should merge with existing attributes correctly', () => {
    ...
    const exampleAttributes = [
      { name: 'id', type: 'String', description: 'Attribute discovered from JSON example' },
      { name: 'display_name', type: 'String', description: 'Attribute discovered from JSON example' },
      { name: 'locked', type: 'Boolean', description: 'Attribute discovered from JSON example' }
    ];

    const merged = analyzer.mergeWithExistingAttributes(existingAttributes, exampleAttributes);

    expect(merged).toHaveLength(4); // 2 existing + 2 new
    expect(merged.find(a => a.name === 'id')?.description).toBe('Account ID'); // Existing kept
    expect(merged.find(a => a.name === 'username')?.description).toBe('Account username'); // Existing kept
    expect(merged.find(a => a.name === 'display_name')?.description).toContain('missing from entity definition');
    expect(merged.find(a => a.name === 'locked')?.description).toContain('missing from entity definition');
  });
});