...
class MethodParser {
  ...
  public parseAllMethods(): ApiMethodsFile[] {
    ...
    for (const id of source.list('method')) {
      ...
      try {
        const methodFile = this.parseMethodFile(source.read('method', id), id);
        if (methodFile) {
          methodFiles.push(methodFile);
        }
      } catch (error) {
        throw new Error(
          `Error parsing method file ${id}: ${(error as Error).message}`,
          { cause: error }
        );
      }
    }
  ...
  private parseMethodFile(
    ...
    // Tags are derived from the document name
    const fileName = path.basename(documentId, '.md');
    if (!fileName) {
      console.warn(`No filename found in ${documentId}`);
      return null;
    }
  ...
  private parseMethods(content: string): ApiMethod[] {
    ...
    // Split on level 2 headers. Some docs include explicit anchors ({#...}) while
    // others rely on implicit heading slugs generated by Hugo. Fenced samples may
    // contain lines that look like headings, so boundaries are matched outside fences.
  ...
  private parseMethodSection(section: string): ApiMethod | null {
    // Extract method name and optional anchor from the heading.
    // Supports both explicit anchors (## Name {#anchor}) and implicit anchors
    // where Hugo generates the slug from heading text.
    const nameMatch = section.match(/^#{2,3}\s+(.+?)(?:\s*\{#([^}]+)\})?\s*$/m);
    if (!nameMatch) return null;

    const name = nameMatch[1].trim();
    const anchor = nameMatch[2]
      ? nameMatch[2].trim()
      : this.generateAnchorFromHeading(name);
  ...
  private generateAnchorFromHeading(heading: string): string {
    return heading
      .replace(/\{\{%[^%]+%\}\}/g, '')
      .trim()
      .toLowerCase()
